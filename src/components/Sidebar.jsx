...
function Sidebar() {
  ...
  return (
    <div className="w-[15%] h-screen ">
      <div className="p-3">
        <button  onClick={()=>dispatch(setOpen(true))} className="flex item-center gap-2 p-4 rounded-2xl hover:shadow bg-[#C2E7FF]">
          <LuPencil size={"24px"} />
          Compose  
        </button>
        <div className="text-gray-500">
          {
            sidebarItems.map((item, index) => {
              return(
            <div className="flex item-center gap-4 pl-6 py-1 rounded-r-full hover:cursor-pointer hover:bg-gray-200 my-2">
              {item.icon}
              <p>{item.text}</p>
            </div>
              );
